...
// OP3NF1XER: OpenClaw-Alma-v2 with SSH preconfiguration
// Deployment: Railway template with OpenSSH client integration
console.log("[wrapper] OpenClaw-Alma-v2 - OP3NF1XER Governance");
...
// OP3NF1XER: SSH Environment Setup
const SSH_DIR = process.env.SSH_DIR?.trim() || "/data/.ssh";
const SSH_KNOWN_HOSTS = process.env.SSH_KNOWN_HOSTS?.trim();
const SSH_CONFIG = process.env.SSH_CONFIG?.trim();
const SSH_PRIVATE_KEY = process.env.SSH_PRIVATE_KEY?.trim();
...
app.get("/setup/api/debug", requireSetupAuth, async (_req, res) => {
  ...
  // OP3NF1XER: Check SSH status
  const sshStatus = {
    sshDir: SSH_DIR,
    sshDirExists: fs.existsSync(SSH_DIR),
    knownHostsExists: fs.existsSync(path.join(SSH_DIR, "known_hosts")),
    configExists: fs.existsSync(path.join(SSH_DIR, "config")),
    ed25519KeyExists: fs.existsSync(path.join(SSH_DIR, "id_ed25519")),
    rsaKeyExists: fs.existsSync(path.join(SSH_DIR, "id_rsa")),
  };
...
// OP3NF1XER: SSH Setup Function
async function setupSSH() {
  try {
    console.log("[wrapper] Setting up SSH environment...");
    
    // Create SSH directory
    fs.mkdirSync(SSH_DIR, { recursive: true });
    fs.chmodSync(SSH_DIR, 0o700);
    
    // Configure known_hosts if provided
    if (SSH_KNOWN_HOSTS) {
      const knownHostsPath = path.join(SSH_DIR, "known_hosts");
      try {
        const decoded = Buffer.from(SSH_KNOWN_HOSTS, 'base64').toString('utf8');
        fs.writeFileSync(knownHostsPath, decoded, { mode: 0o644 });
        console.log("[wrapper] SSH known_hosts configured");
      } catch (e) {
        console.warn(`[wrapper] Failed to configure known_hosts: ${e.message}`);
      }
    }
    
    // Configure SSH config if provided
    if (SSH_CONFIG) {
      const configPath = path.join(SSH_DIR, "config");
      try {
        const decoded = Buffer.from(SSH_CONFIG, 'base64').toString('utf8');
        fs.writeFileSync(configPath, decoded, { mode: 0o600 });
        console.log("[wrapper] SSH config configured");
      } catch (e) {
        console.warn(`[wrapper] Failed to configure SSH config: ${e.message}`);
      }
    }
    
    // Configure ED25519 private key if provided
    if (SSH_PRIVATE_KEY) {
      const keyPath = path.join(SSH_DIR, "id_ed25519");
      try {
        const decoded = Buffer.from(SSH_PRIVATE_KEY, 'base64').toString('utf8');
        fs.writeFileSync(keyPath, decoded, { mode: 0o600 });
        console.log("[wrapper] SSH ED25519 private key configured");
      } catch (e) {
        console.warn(`[wrapper] Failed to configure SSH key: ${e.message}`);
      }
    }
    
    // Run ssh-setup script if available
    const sshSetupPath = "/usr/local/bin/ssh-setup";
    if (fs.existsSync(sshSetupPath)) {
      console.log("[wrapper] Running ssh-setup script...");
      try {
        await runCmd("bash", [sshSetupPath], { timeoutMs: 30_000 });
        console.log("[wrapper] ssh-setup complete");
      } catch (err) {
        console.warn(`[wrapper] ssh-setup failed (continuing): ${String(err)}`);
      }
    }
    
    console.log(`[wrapper] SSH directory: ${SSH_DIR}`);
  } catch (err) {
    console.warn(`[wrapper] SSH setup failed (continuing): ${String(err)}`);
  }
}
...
const server = app.listen(PORT, "0.0.0.0", async () => {
  ...
  // OP3NF1XER: Setup SSH environment
  await setupSSH();
